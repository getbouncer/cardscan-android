...
abstract class CardScanBaseActivity :
     ...
     */
    override suspend fun onResult(result: String) = launch(Dispatchers.Main) {
        resultListener.cardScanned(pan = result)
        closeScanner()
    }.let { }
    ...
    override suspend fun onInterimResult(result: MainLoopAggregator.InterimResult) = launch(Dispatchers.Main) {
        ...
        result.analyzerResult.detectedBoxes.let { detectionBoxes ->
            if (Config.isDebug) {
